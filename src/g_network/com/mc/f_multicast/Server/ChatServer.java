package g_network.com.mc.f_multicast.Server;
...
public class ChatServer {
	...
	public ChatServer() { // 기본 생성자, 서버 열준비.
	...
	private void read(Socket socket) { // 소켓 데이터 읽어오기
		...
		Thread thread = new Thread(new Runnable() { // run 메서드를 구현해야하는 Runnable 인터페이스
			...
			public void run() {
				...
				try (BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()))) { // socket의
																												// 스트림데이터
																												// 받아오기
					while (true) { // 무한반복문
						
						String message = br.readLine(); // message 변수에 BufferedReader로 읽어온 문자행을 초기화
						boolean isWhisper = message.contains("/To "); // message에 to가 포함되었을 경우를 논리값으로 저장

						if (isWhisper == true) {// whisper가 true라면,
							// broadcast
							// user id 부분을 잘라서 변수에 저장해야함.
							int indexOfUserId = message.indexOf("//");
							String whisperMessage = message.substring(indexOfUserId + 1); // userId인덱스의 다음 인덱스부터 끝가지
																							// 추출하여 whisperMessage변수에
																							// 초기화
							broadcast(whisperMessage); // whisperMessage를 매개변수로 갖는 broadcast 메서드 실행
						} else { // whisper가 false라면,
									// multicast
							multicast(message); // message를 매개변수로 갖는 broadcast 메서드 실행
						}
	...
	protected void broadcast(String message) { // message를 파라미터로 받는다.
		...
		// 귓속말을 받을 userId를 어떻게 특정하지?
		for (Socket socket : socketList) { // socket 변수를 socketList만큼 돌린다.

			try {
				PrintWriter writer = new PrintWriter(socket.getOutputStream()); // PrintWriter타입의 writer변수에
																				// soket.getOutputStream 값을 넣어줍니다. 반대편으로
																				// 발사(출력)할 총알 장전 정도로 이해하면 될듯?
	...
	protected void multicast(String message) {
